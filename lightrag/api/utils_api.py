...
def get_admin_auth_dependency(api_key: Optional[str] = None):
    """
    Create an admin-only authentication dependency that restricts access to admin users only.
    
    This function builds on the combined auth dependency and adds an additional role check.
    
    Args:
        api_key (Optional[str]): API key for validation
    
    Returns:
        Callable: A dependency function that implements admin role validation
    """
    oauth2_scheme = OAuth2PasswordBearer(
        tokenUrl="login", auto_error=False, description="OAuth2 Password Authentication"
    )
    ...
    async def admin_dependency(
        ...
        # 2. If no auth configured and no API key, allow (backward compatibility)
        if not auth_configured and not api_key:
            return
        
        # 3. Check API key first if provided
        if api_key and api_key_header_value == api_key:
            return  # API key grants admin access
        
        # 4. Validate JWT token and check for admin role
        if token:
            try:
                token_info = auth_handler.validate_token(token)
        ...
        # 5. No valid credentials provided
        if auth_configured:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Authentication required. Admin privileges needed.",
            )
