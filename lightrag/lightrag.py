...
from lightrag.constants import (
    ...
    DEFAULT_ENABLE_ENTITY_MERGE_VERIFICATION,
    DEFAULT_ENTITY_CONTEXT_SIMILARITY_WEIGHT,
)
...
class LightRAG:
    ...
    # Entity merging verification
    # ---

    enable_entity_merge_verification: bool = field(
        default=get_env_value(
            "ENABLE_ENTITY_MERGE_VERIFICATION",
            DEFAULT_ENABLE_ENTITY_MERGE_VERIFICATION,
            lambda x: x.lower() == "true" if isinstance(x, str) else bool(x),
        )
    )
    """Enable semantic verification before merging entities with the same name."""
    ...
    """Minimum similarity threshold (0-1) for merging entities. Higher values are more strict."""

    entity_context_similarity_weight: float = field(
        default=get_env_value(
            "ENTITY_CONTEXT_SIMILARITY_WEIGHT",
            DEFAULT_ENTITY_CONTEXT_SIMILARITY_WEIGHT,
            float,
        )
    )
    """Weight for context similarity (file paths, sources) vs description similarity when merging entities."""
    ...
    async def apipeline_enqueue_documents(
        ...
        file_paths: str | list[str] | None = None,
        accreditation_levels: int | list[int] | None = None,
        ...
        Args:
            ...
            ids: list of unique document IDs, if not provided, MD5 hash IDs will be generated
            file_paths: list of file paths corresponding to each document, used for citation
            accreditation_levels: Accreditation level(s) for document(s), 0-999, default 0 (public)
            services: Service(s)/department(s) for document(s), one of: DOCT, DRI, DT, DREC, DAF, CABINET, DAP
        ...
        # Handle accreditation_levels
        if accreditation_levels is not None:
            if isinstance(accreditation_levels, int):
                accreditation_levels = [accreditation_levels] * len(input)
            if len(accreditation_levels) != len(input):
                raise ValueError(
                    "Number of accreditation levels must match the number of documents"
                )
        else:
            # Default accreditation level is 0 (public)
            accreditation_levels = [0] * len(input)

        # Handle services
        if services is not None:
            if isinstance(services, str):
                services = [services] * len(input)
            if len(services) != len(input):
                raise ValueError(
                    "Number of services must match the number of documents"
                )
        else:
            # Default service is None
            services = [None] * len(input)
        ...
        if ids is not None:
            ...
            unique_contents = {}
            for id_, doc, path, acc_level, service in zip(ids, input, file_paths, accreditation_levels, services):
                cleaned_content = sanitize_text_for_encoding(doc)
                if cleaned_content not in unique_contents:
                    unique_contents[cleaned_content] = (id_, path, acc_level, service)

            # Reconstruct contents with unique content
            contents = {
                id_: {"content": content, "file_path": file_path, "accreditation_level": acc_level, "service": service}
                for content, (id_, file_path, acc_level, service) in unique_contents.items()
            }
        else:
            # Clean input text and remove duplicates in one pass
            unique_content_with_paths = {}
            for doc, path, acc_level, service in zip(input, file_paths, accreditation_levels, services):
                cleaned_content = sanitize_text_for_encoding(doc)
                if cleaned_content not in unique_content_with_paths:
                    unique_content_with_paths[cleaned_content] = (path, acc_level, service)
            ...
            contents = {
                compute_mdhash_id(content, prefix="doc-"): {
                    "content": content,
                    "file_path": path,
                    "accreditation_level": acc_level,
                    "service": service,
                }
                for content, (path, acc_level, service) in unique_content_with_paths.items()
            }
        ...
        new_docs: dict[str, Any] = {
            id_: {
                ...
                "accreditation_level": content_data.get("accreditation_level", 0),  # Store accreditation level
                "service": content_data.get("service"),  # Store service/department
            }
        ...
        full_docs_data = {
            doc_id: {
                ...
                "accreditation_level": contents[doc_id].get("accreditation_level", 0),
            }
                ...
                async def process_document(
                    ...
                    async with semaphore:
                        ...
                        try:
                            ...
                            # Get accreditation level from document (default to 0)
                            accreditation_level = content_data.get("accreditation_level", 0)
                            # Get service from document (default to None)
                            ...
                            chunks: dict[str, Any] = {
                                compute_mdhash_id(dp["content"], prefix="chunk-"): {
                                    **dp,
                                    ...
                                    "accreditation_level": accreditation_level,  # Add accreditation level to each chunk
                                    "service": service,  # Add service to each chunk
    ...
    async def ainsert_custom_kg(
        ...
        try:
            ...
            for relationship_data in custom_kg.get("relationships", []):
                ...
                accreditation_level = relationship_data.get("accreditation_level", 0)
                service = relationship_data.get("service")
            ...
            data_for_vdb = {
                compute_mdhash_id(dp["entity_name"], prefix="ent-"): {
                    ...
                    "accreditation_level": dp.get("accreditation_level", 0),
                    "service": dp.get("service"),
                }
