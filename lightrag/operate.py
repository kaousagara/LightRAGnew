...
async def _process_extraction_result(
    ...
    Args:
        ...
        accreditation_level (int): The accreditation level for extracted entities and relations
    ...
    for record in fixed_records:
        ...
        entity_data = await _handle_single_entity_extraction(
            record_attributes, chunk_key, timestamp, file_path, accreditation_level
        )
...
async def _rebuild_from_extraction_result(
    ...
    # Get chunk data for file_path and accreditation_level from storage
    ...
    accreditation_level = (
        chunk_data.get("accreditation_level", 0)
        if chunk_data
        else 0
    )
        ...
        try:
            ...
            vdb_data = {
                entity_vdb_id: {
                    ...
                    "accreditation_level": updated_entity_data.get("accreditation_level", 0),
                    "service": updated_entity_data.get("service"),
                }
            }
    ...
    try:
        ...
        vdb_data = {
            rel_vdb_id: {
                ...
                "accreditation_level": updated_relationship_data.get("accreditation_level", 0),
                "service": updated_relationship_data.get("service"),
            }
        }
...
async def _should_merge_entities(
    entity_name: str,
    ...
    """
    Determine if entities with the same name should be merged based on semantic similarity.
    
    This function performs a comprehensive similarity check including:
    1. Description semantic similarity (via embeddings)
    2. Context similarity (file paths and source IDs)
    3. Entity type compatibility
    
    Args:
        entity_name: Name of the entity being evaluated
        new_nodes_data: List of new entity data dictionaries to be merged
        existing_node: Existing entity node from the knowledge graph (None if doesn't exist)
        global_config: Global configuration containing embedding function and thresholds
        
    Returns:
        Tuple of (should_merge: bool, similarity_score: float)
        - should_merge: True if entities should be merged, False if they should be kept separate
        - similarity_score: Computed similarity score (0-1)
    """
    # If verification is disabled, always merge
    if not global_config.get("enable_entity_merge_verification", True):
        return True, 1.0
    
    # If no existing node, always allow merge (it's a new entity)
    if existing_node is None:
        return True, 1.0
    
    # Get configuration parameters
    similarity_threshold = global_config.get("entity_merge_similarity_threshold", 0.85)
    context_weight = global_config.get("entity_context_similarity_weight", 0.3)
    description_weight = 1.0 - context_weight
    embedding_func = global_config.get("embedding_func")
    
    # If no embedding function is available, fall back to simple type matching
    if embedding_func is None:
        logger.warning(
            f"No embedding function available for entity merge verification of '{entity_name}'. "
            "Falling back to entity type matching."
        )
        # Check if entity types are compatible
        existing_type = existing_node.get("entity_type", "UNKNOWN")
        new_types = [node.get("entity_type", "UNKNOWN") for node in new_nodes_data]
        type_match = existing_type in new_types or existing_type == "UNKNOWN"
        return type_match, 1.0 if type_match else 0.0
    
    try:
        # 1. Calculate description similarity using embeddings
        existing_description = existing_node.get("description", "")
        new_descriptions = [node.get("description", "") for node in new_nodes_data]
        
        # Combine new descriptions for comparison
        combined_new_description = " ".join(new_descriptions[:3])  # Use first 3 to avoid too long text
        
        # Get embeddings
        if existing_description and combined_new_description:
            embeddings = await embedding_func([existing_description, combined_new_description])
            embeddings_array = np.array(embeddings)
            
            # Calculate cosine similarity
            existing_emb = embeddings_array[0]
            new_emb = embeddings_array[1]
            
            # Normalize vectors
            existing_norm = existing_emb / (np.linalg.norm(existing_emb) + 1e-10)
            new_norm = new_emb / (np.linalg.norm(new_emb) + 1e-10)
            
            # Cosine similarity
            description_similarity = float(np.dot(existing_norm, new_norm))
            description_similarity = max(0.0, min(1.0, description_similarity))  # Clamp to [0, 1]
        else:
            # If descriptions are empty, use low similarity
            description_similarity = 0.5
        
        # 2. Calculate context similarity (file paths and source IDs)
        existing_file_paths = set(existing_node.get("file_path", "").split(GRAPH_FIELD_SEP))
        existing_source_ids = set(existing_node.get("source_id", "").split(GRAPH_FIELD_SEP))
        
        new_file_paths = set()
        new_source_ids = set()
        for node in new_nodes_data:
            new_file_paths.add(node.get("file_path", "unknown_source"))
            new_source_ids.add(node.get("source_id", ""))
        
        # Calculate Jaccard similarity for file paths
        file_path_intersection = len(existing_file_paths & new_file_paths)
        file_path_union = len(existing_file_paths | new_file_paths)
        file_path_similarity = file_path_intersection / file_path_union if file_path_union > 0 else 0.0
        
        # Calculate Jaccard similarity for source IDs
        source_id_intersection = len(existing_source_ids & new_source_ids)
        source_id_union = len(existing_source_ids | new_source_ids)
        source_id_similarity = source_id_intersection / source_id_union if source_id_union > 0 else 0.0
        
        # Average context similarity
        context_similarity = (file_path_similarity + source_id_similarity) / 2.0
        
        # 3. Calculate final weighted similarity score
        final_similarity = (
        ...
        should_merge = final_similarity >= similarity_threshold
        
        # Log the decision for transparency
        if not should_merge:
            logger.info(
                f"Entity merge verification for '{entity_name}': "
                f"similarity={final_similarity:.3f} (threshold={similarity_threshold:.3f}) - "
                f"desc={description_similarity:.3f}, context={context_similarity:.3f} "
                f"(files={file_path_similarity:.3f}, sources={source_id_similarity:.3f}) - "
                f"Decision: {'MERGE' if should_merge else 'KEEP SEPARATE'}"
            )
        
        return should_merge, final_similarity
        
    except Exception as e:
        logger.error(f"Error in entity merge verification for '{entity_name}': {e}")
        logger.error(traceback.format_exc())
        # On error, fall back to merging (conservative approach)
        return True, 0.0
...
async def _merge_nodes_then_upsert(
    ...
    This function now includes entity merge verification to prevent incorrect merging of
    entities with the same name but different contexts or meanings.
    """
    already_entity_types = []
    already_source_ids = []
    already_description = []
    already_file_paths = []
    already_accreditation_levels = []
    already_services = []

    already_node = await knowledge_graph_inst.get_node(entity_name)
    
    # Perform entity merge verification if enabled
    if already_node and global_config.get("enable_entity_merge_verification", True):
        should_merge, similarity_score = await _should_merge_entities(
            entity_name,
            nodes_data,
            already_node,
            global_config
        ...
        # If entities should NOT be merged, create a new entity with a contextual suffix
        if not should_merge:
            # Generate a contextual suffix based on the primary document source
            primary_file_path = nodes_data[0].get("file_path", "unknown_source") if nodes_data else "unknown_source"
            # Extract a meaningful part from the file path (last part without extension)
            from pathlib import Path
            file_stem = Path(primary_file_path).stem if primary_file_path != "unknown_source" else "alt"
            
            # Create a shorter, cleaner suffix (max 20 chars)
            suffix = file_stem[:20].replace(" ", "_").replace("-", "_")
            new_entity_name = f"{entity_name}_{suffix}"
            
            logger.info(
                f"Entity '{entity_name}' similarity too low ({similarity_score:.3f}). "
                f"Creating separate entity: '{new_entity_name}'"
            )
            
            # Update the entity name in nodes_data to use the new name
            for node in nodes_data:
                node["entity_name"] = new_entity_name
            
            # Recursively call this function with the new entity name
            # (this will create a new entity since it doesn't exist yet)
    ...
    # Proceed with normal merging process
    if already_node:
        ...
        # Collect existing accreditation_level (take the most restrictive, i.e., minimum)
        if already_node.get("accreditation_level") is not None:
            already_accreditation_levels.append(already_node["accreditation_level"])
        
        # Collect existing services
        if already_node.get("service"):
            already_services.extend(already_node["service"].split(GRAPH_FIELD_SEP))
    ...
    # Calculate accreditation_level: take minimum (most restrictive) from all sources
    all_accreditation_levels = already_accreditation_levels + [
        dp.get("accreditation_level", 0) for dp in nodes_data if dp.get("accreditation_level") is not None
    ]
    accreditation_level = min(all_accreditation_levels) if all_accreditation_levels else 0
    
    # Collect all unique services from all sources
    all_services = already_services + [
        dp.get("service") for dp in nodes_data if dp.get("service")
    ]
    service = GRAPH_FIELD_SEP.join(set(filter(None, all_services))) if all_services else None
...
async def _merge_edges_then_upsert(
    ...
    already_description = []
    already_keywords = []
    already_keyword_scores = {}  # Track existing keyword scores
    already_file_paths = []
    already_accreditation_levels = []
    already_services = []
    ...
    if await knowledge_graph_inst.has_edge(src_id, tgt_id):
        ...
        if already_edge:
            ...
            # Get existing keyword scores for backward compatibility
            import json
            if already_edge.get("keyword_scores"):
                try:
                    already_keyword_scores = json.loads(already_edge["keyword_scores"])
                except (json.JSONDecodeError, TypeError):
                    already_keyword_scores = {}
            
            # Collect existing accreditation_level (take the most restrictive, i.e., minimum)
            if already_edge.get("accreditation_level") is not None:
                already_accreditation_levels.append(already_edge["accreditation_level"])
            
            # Collect existing services
            if already_edge.get("service"):
                already_services.extend(already_edge["service"].split(GRAPH_FIELD_SEP))
    ...
    # Calculate keyword scores based on frequency and weight
    # This enables intelligent relationship type selection
    from collections import Counter
    keyword_counter = Counter()
    keyword_weights = {}  # Track accumulated weights for each keyword
    
    # Initialize with existing keyword scores (for backward compatibility and merging)
    for kw, score in already_keyword_scores.items():
        keyword_scores_temp = {kw: score}
        # Decompose score back to approximate frequency and weight
        # Assume score = frequency * weight, estimate frequency from existing data
        keyword_counter[kw] = 1  # At least one occurrence from existing data
        keyword_weights[kw] = score  # Use existing score as accumulated weight
    ...
    for keyword_str in already_keywords:
        if keyword_str:  # Skip empty strings
            for k in keyword_str.split(","):
                k_clean = k.strip()
                if k_clean:
                    if k_clean not in already_keyword_scores:
                        # Only count if not already in scores (avoid double counting)
                        keyword_counter[k_clean] += 1
                        keyword_weights[k_clean] = keyword_weights.get(k_clean, 0.0) + 1.0
    
    # Process new keywords from edges_data with their weights
    for edge in edges_data:
        if edge.get("keywords"):
            edge_weight = edge.get("weight", 1.0)
            for k in edge["keywords"].split(","):
                k_clean = k.strip()
                if k_clean:
                    keyword_counter[k_clean] += 1
                    keyword_weights[k_clean] = keyword_weights.get(k_clean, 0.0) + edge_weight
    
    # Calculate final scores: frequency * accumulated_weight
    keyword_scores = {}
    for kw in set(list(keyword_counter.keys()) + list(already_keyword_scores.keys())):
        freq = keyword_counter.get(kw, 0)
        weight = keyword_weights.get(kw, 0.0)
        if freq > 0 or kw in already_keyword_scores:
            keyword_scores[kw] = freq * weight if freq > 0 else already_keyword_scores.get(kw, 0.0)
    
    # Join all unique keywords sorted by score (highest first), then alphabetically for ties
    sorted_keywords = sorted(
        keyword_scores.keys(), 
        key=lambda k: (-keyword_scores[k], k)  # Negative for descending, then alphabetical
    )
    keywords = ",".join(sorted_keywords)
    
    # Store keyword scores as JSON for relationship type determination
    import json
    keyword_scores_json = json.dumps(keyword_scores) if keyword_scores else None
    ...
    # Calculate accreditation_level: take minimum (most restrictive) from all sources
    all_accreditation_levels = already_accreditation_levels + [
        dp.get("accreditation_level", 0) for dp in edges_data if dp.get("accreditation_level") is not None
    ]
    accreditation_level = min(all_accreditation_levels) if all_accreditation_levels else 0
    
    # Collect all unique services from all sources
    all_services = already_services + [
        dp.get("service") for dp in edges_data if dp.get("service")
    ]
    service = GRAPH_FIELD_SEP.join(set(filter(None, all_services))) if all_services else None
    ...
    await knowledge_graph_inst.upsert_edge(
        ...
        edge_data=dict(
            ...
            description=description,
            keywords=keywords,
            keyword_scores=keyword_scores_json,  # Add keyword scores for intelligent type selection
            source_id=source_id,
            file_path=file_path,
            accreditation_level=accreditation_level,
            service=service,
                    ...
                    if entity_vdb is not None and entity_data:
                        data_for_vdb = {
                            compute_mdhash_id(
                                ...
                                "accreditation_level": entity_data.get("accreditation_level", 0),
                    ...
                    if relationships_vdb is not None:
                        data_for_vdb = {
                            compute_mdhash_id(
                                ...
                                "accreditation_level": edge_data.get("accreditation_level", 0),
...
async def extract_entities(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        # Get accreditation level from chunk data or use default
        accreditation_level = chunk_dp.get("accreditation_level", 0)
        ...
        # Process initial extraction with file path and accreditation level
        ...
        if entity_extract_max_gleaning > 0:
            ...
            # Process gleaning result separately with file path and accreditation level
    ...
    # Process chunks in batches of MAX_ASYNC to control memory usage
    # and ensure that we never exceed the MAX_ASYNC limit
    chunk_results = []
    
    # Process chunks in batches
    for batch_start in range(0, len(ordered_chunks), chunk_max_async):
        batch_end = min(batch_start + chunk_max_async, len(ordered_chunks))
        ...
        # Wait for all tasks in this batch to complete or for first exception
        done, pending = await asyncio.wait(tasks, return_when=asyncio.FIRST_EXCEPTION)
        ...
        # If any task failed, cancel all pending tasks and raise the first exception
        if first_exception is not None:
            # Cancel all pending tasks in this batch
            for pending_task in pending:
                pending_task.cancel()
    ...
    # Return all chunk_results for later processing in merge_nodes_and_edges
...
async def _get_vector_context(
    ...
    try:
        ...
        user_accreditation_level = getattr(query_param, "user_accreditation_level", 999)
        
        valid_chunks = []
        filtered_count = 0
        for result in results:
            if "content" in result:
                # Check accreditation level access
                chunk_accreditation = result.get("accreditation_level", 0)
                if chunk_accreditation > user_accreditation_level:
                    filtered_count += 1
                    continue  # Skip chunks above user's accreditation level
        ...
        if filtered_count > 0:
            logger.info(
                f"Filtered {filtered_count} chunks due to accreditation level restrictions (user_level: {user_accreditation_level})"
            )
        logger.info(
...
async def _get_node_data(
    ...
    # Filter entities by accreditation level
    user_accreditation_level = getattr(query_param, "user_accreditation_level", 999)
    
    node_datas = []
    filtered_entity_count = 0
    for k, n, d in zip(results, node_datas_raw := [nodes_dict.get(nid) for nid in node_ids], node_degrees):
        if n is not None:
            entity_accreditation = n.get("accreditation_level", 0)
            if entity_accreditation > user_accreditation_level:
                filtered_entity_count += 1
                continue  # Skip entities above user's accreditation level
    ...
    if filtered_entity_count > 0:
        logger.info(
            f"Filtered {filtered_entity_count} entities due to accreditation level restrictions (user_level: {user_accreditation_level})"
        )
...
async def _find_related_text_unit_from_entities(
    ...
    # Step 6: Build result chunks with valid data, filter by accreditation level, and update chunk tracking
    user_accreditation_level = getattr(query_param, "user_accreditation_level", 999)
    result_chunks = []
    ...
    for i, (chunk_id, chunk_data) in enumerate(zip(unique_chunk_ids, chunk_data_list)):
        if chunk_data is not None and "content" in chunk_data:
            # Check accreditation level access
            chunk_accreditation = chunk_data.get("accreditation_level", 0)
            if chunk_accreditation > user_accreditation_level:
    ...
    if filtered_chunk_count > 0:
        logger.info(
            f"Filtered {filtered_chunk_count} entity-related chunks due to accreditation level restrictions"
        )
...
async def _get_edge_data(
    ...
    # Filter relations by accreditation level
    user_accreditation_level = getattr(query_param, "user_accreditation_level", 999)
    ...
    for k in results:
        ...
        if edge_props is not None:
            # Check accreditation level access
            relation_accreditation = edge_props.get("accreditation_level", 0)
            if relation_accreditation > user_accreditation_level:
                filtered_relation_count += 1
                continue  # Skip relations above user's accreditation level
    ...
    if filtered_relation_count > 0:
        logger.info(
            f"Filtered {filtered_relation_count} relations due to accreditation level restrictions (user_level: {user_accreditation_level})"
        )
...
async def _find_related_text_unit_from_relations(
    ...
    if filtered_chunk_count > 0:
        logger.info(
            f"Filtered {filtered_chunk_count} relation-related chunks due to accreditation level restrictions"
        )
