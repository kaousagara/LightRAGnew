...
class Neo4JStorage(BaseGraphStorage):
    ...
    def _normalize_relationship_type(self, rel_type: str) -> str:
        """Normalize relationship type to avoid synonyms and ensure Neo4j compatibility.
        
        Args:
            rel_type: Raw relationship type string
            
        Returns:
            Normalized relationship type safe for Neo4j
        """
    ...
    def _generate_relationship_type_from_data(
        self, 
        edge_data: dict[str, str],
        source_entity_type: str | None = None,
        target_entity_type: str | None = None
    ) -> str:
        """Generate relationship type based on weighted keywords or entity types.
        
        This method works without database calls and can be used within transactions.
        Now supports weighted keyword scoring for more intelligent type selection.
        
        Priority:
        1. Highest-scoring keyword from keyword_scores (frequency × weight)
        2. First keyword from keywords list (backward compatibility)
        3. Entity types combined (source_type_to_target_type)
        4. Default: "RELATED_TO"
        
        Args:
            edge_data: Edge data containing keywords, keyword_scores, and other properties
            source_entity_type: Optional type of source entity
            target_entity_type: Optional type of target entity
            
        Returns:
            Generated relationship type
        """
        # Priority 1: Use weighted keyword scores if available
        keyword_scores_json = edge_data.get("keyword_scores")
        if keyword_scores_json:
            try:
                import json
                keyword_scores = json.loads(keyword_scores_json)
                if keyword_scores:
                    # Select keyword with highest score
                    best_keyword = max(keyword_scores.items(), key=lambda x: x[1])[0]
                    if best_keyword:
                        return self._normalize_relationship_type(best_keyword)
            except (json.JSONDecodeError, TypeError, ValueError) as e:
                logger.debug(f"Failed to parse keyword_scores, falling back to keywords list: {e}")
        
        # Priority 2: Fallback to first keyword (backward compatibility)
        keywords = edge_data.get("keywords", "").strip()
        if keywords:
            # Keywords are now sorted by score, so first is the most relevant
            keyword_parts = keywords.split(",")  # Changed from GRAPH_FIELD_SEP to comma
            first_keyword = keyword_parts[0].strip() if keyword_parts else ""
            if first_keyword:
                return self._normalize_relationship_type(first_keyword)
        
        # Priority 3: Use entity types
        if source_entity_type and target_entity_type:
            rel_type = f"{source_entity_type}_to_{target_entity_type}"
            return self._normalize_relationship_type(rel_type)
    ...
    async def get_edge(
        ...
        Returns:
            dict: Edge properties if found (including 'relationship_type'), None if not found
                ...
                try:
                    ...
                    if records:
                        try:
                            ...
                            relationship_type = records[0].get("relationship_type", "RELATED_TO")
                            edge_result["relationship_type"] = relationship_type
    ...
    async def get_edges_batch(
        ...
        Supports all relationship types and returns the relationship type in results.

        Args:
            pairs: List of dictionaries, e.g. [{"src": "node1", "tgt": "node2"}, ...]

        Returns:
            A dictionary mapping (src, tgt) tuples to their edge properties (including 'relationship_type').
        """
        workspace_label = self._get_workspace_label()
        async with self._driver.session(
            database=self._DATABASE, default_access_mode="READ"
            ...
            RETURN pair.src AS src_id, pair.tgt AS tgt_id, collect(properties(r)) AS edges, collect(type(r)) AS rel_types
            """
            ...
            async for record in result:
                ...
                if edges and len(edges) > 0:
                    edge_props = edges[0]
                    relationship_type = rel_types[0] if rel_types and len(rel_types) > 0 else "RELATED_TO"
                    edge_props["relationship_type"] = relationship_type
    ...
    async def upsert_edge(
        ...
        The relationship type is dynamically generated based on entity types and keywords.
        ...
        try:
            ...
            async with self._driver.session(database=self._DATABASE) as session:
                ...
                async def execute_upsert(tx: AsyncManagedTransaction):
                    ...
                    RETURN source.entity_type as source_type, target.entity_type as target_type
                    """
                    
                    type_result = await tx.run(
                        fetch_types_query,
                        source_entity_id=source_node_id,
                        target_entity_id=target_node_id,
                    )
                    ...
                    target_type = None
                    if type_record:
                        source_type = type_record.get("source_type")
                        target_type = type_record.get("target_type")
                    
                    relationship_type = self._generate_relationship_type_from_data(
                        edge_data, source_type, target_type
                    )
