...
const useGraphStoreBase = create<GraphState>()((set, get) => ({
  ...
  setSelectedNode: (nodeId: string | null, moveToSelectedNode?: boolean) =>
    set({ selectedNode: nodeId, selectedNodes: nodeId ? new Set([nodeId]) : new Set(), moveToSelectedNode }),
  ...
  toggleNodeSelection: (nodeId: string) => {
    ...
    set({ selectedNodes: newSelection, selectedNode: newSelection.size === 1 ? Array.from(newSelection)[0] : null })
  },
  
  setSelectedNodes: (nodeIds: Set<string>) => 
    set({ selectedNodes: nodeIds, selectedNode: nodeIds.size === 1 ? Array.from(nodeIds)[0] : null }),
  
  setFocusedNode: (nodeId: string | null) => set({ focusedNode: nodeId }),
  setSelectedEdge: (edgeId: string | null) => set({ selectedEdge: edgeId, selectedEdges: edgeId ? new Set([edgeId]) : new Set() }),
  ...
  toggleEdgeSelection: (edgeId: string) => {
    ...
    set({ selectedEdges: newSelection, selectedEdge: newSelection.size === 1 ? Array.from(newSelection)[0] : null })
  },
  
  setSelectedEdges: (edgeIds: Set<string>) =>
    set({ selectedEdges: edgeIds, selectedEdge: edgeIds.size === 1 ? Array.from(edgeIds)[0] : null }),
