...
import { SERVICE_OPTIONS, type ServiceType } from '@/lib/config'
...
export default function UploadDocumentsDialog({ onDocumentsUploaded }: UploadDocumentsDialogProps) {
  ...
  const [accreditationLevel, setAccreditationLevel] = useState<number | undefined>(undefined)
    ...
    <Dialog
      ...
      onOpenChange={(open) => {
        ...
        if (!open) {
          setProgresses({})
          setFileErrors({})
          setAccreditationLevel(undefined)
          setService('')
        }
        setOpen(open)
      }}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <label htmlFor="accreditation-level" className="text-sm font-medium">
                {t('documentPanel.uploadDocuments.accreditationLevel', 'Niveau d\'accréditation')}
              </label>
              <Input
                id="accreditation-level"
                type="number"
                placeholder="0-999"
                min={0}
                max={999}
                value={accreditationLevel ?? ''}
                onChange={(e) => setAccreditationLevel(e.target.value ? Number(e.target.value) : undefined)}
                disabled={isUploading}
              />
            </div>
            <div className="space-y-2">
              <label htmlFor="service" className="text-sm font-medium">
                {t('documentPanel.uploadDocuments.service', 'Service / Direction')}
              </label>
              <Select
                value={service}
                onValueChange={(value: string) => setService(value as ServiceType)}
                disabled={isUploading}
              >
                <SelectTrigger id="service">
                  <SelectValue placeholder={t('documentPanel.uploadDocuments.noService', 'Aucun')} />
                </SelectTrigger>
                <SelectContent>
                  {SERVICE_OPTIONS.map((option) => (
                    <SelectItem key={option} value={option}>
                      {option || t('documentPanel.uploadDocuments.noService', 'Aucun')}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
