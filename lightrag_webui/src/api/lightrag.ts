...
export const insertText = async (
  ...
  const payload: { text: string; accreditation_level?: number; service?: string } = { text }
  if (accreditationLevel !== undefined) payload.accreditation_level = accreditationLevel
...
export const uploadDocument = async (
  ...
  if (accreditationLevel !== undefined) formData.append('accreditation_level', accreditationLevel.toString())
...
export const isUserAdmin = (): boolean => {
  ...
  if (!token) {
    return !useSettingsStore.getState().authConfigured
  }
  
  try {
    const payload = JSON.parse(atob(token.split('.')[1]))
    return payload.role === 'admin' || !useSettingsStore.getState().authConfigured
  } catch {
    return !useSettingsStore.getState().authConfigured
  }
}
